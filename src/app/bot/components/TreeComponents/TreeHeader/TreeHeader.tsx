...
import { DropAbove } from '../DropAbove/DropAbove';
...
class TreeHeaderClass extends Component<TreeHeaderProps, TreeHeaderState> {
    ...
    getInteractionHasChanges = (interactionsPending?: any[]): boolean => {
        ...
        return !!interactionsPending?.find((currInteractionPending => currInteractionPending?._id === this.props.interaction._id))
    };
        ...
        return (
            <InteractionsPendingPublicationContext.Consumer>
                {({interactionsPendingPublication}) => {
                    return (
                         <RoundedBtn
                            btnClass={'btn-rounded pointer ' + btnClass + ` ${this.getCustomColorInteraction(interactionsPendingPublication)}`}
